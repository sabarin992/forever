...
const AdminAdd = () => {
  ...
  //   try {
  //     const res = await axios.post(`${API_BASE_URL}/add_product/`, product, {
  //       headers: {
  ...
  const addProduct = async () => {
    ...
    for (let i = 0; i < product.variants.length; i++) {
      ...
      if (!variant.price || isNaN(variant.price)) {
        toast.error(`Price must be a number for variant ${i + 1}`);
        return;
      }

      if (!variant.stock_quantity || isNaN(variant.stock_quantity)) {
        toast.error(`Stock quantity must be a number for variant ${i + 1}`);
        return;
      }

      const hasImage = variant.images.some((img) => img);
      if (!hasImage) {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE_URL}/add_product/`, formData, {
        headers: {
      ...
      if (res.status === 201) {
        ...
        // Optionally reset crop-related states too
        setImgSrc("");
        setImgElement(null);
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:gap-8 mb-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  const updatedVariants = [...product.variants];
                  updatedVariants[variantIndex].stock_quantity =
                    Number(e.target.value) === 0 ? "" : Number(e.target.value);
                  setProduct({ ...product, variants: updatedVariants });
                }}
              />
