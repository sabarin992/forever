...
const EditCategoryOffer = () => {
  const location = useLocation();
  const { offer, categories } = location.state;

  const [categoryOffer, setCategoryOffer] = useState({
    category: 0,
    discount_percentage: offer.discount_percentage,
    valid_from: offer.valid_from.slice(0, 16),
    valid_to: offer.valid_to.slice(0, 16), // Fixed: was using valid_from instead of valid_to
  });
  ...
  const editProductOffer = async (e) => {
    e.preventDefault();
    ...
    try {
      const res = await api.put(`/category_offers/${offer.id}/`, categoryOffer);
      console.log(res.data.data);
      toast.success(res.data.message);
      if (res.status === 200) {
        toast.success("Category offer edited successfully!");
      }
    } catch (error) {
      console.log(error);
  ...
  useEffect(() => {
    if (categories.length > 0 && categoryOffer.category === 0) {
      setCategoryOffer({
        ...categoryOffer,
  ...
  return (
    <>
      <h1 className="text-2xl font-bold mb-6">Edit Category Offer</h1>
      <form onSubmit={editProductOffer} className="max-w-4xl">
...
//     discount_percentage: offer.discount_percentage,
//     valid_from: offer.valid_from.slice(0, 16),
//     valid_to: offer.valid_from.slice(0, 16),
//   });

//   const editProductOffer = async (e) => {
//     e.preventDefault();
//     if (!categoryOffer.discount_percentage){
//         toast.error('Please enter any value to discount %')
//         return
//     }
...
//       toast.success(res.data.message);
//       if (res.status === 200) {
//         toast.success("Product offer Edited successfully!");
//       }
//     } catch (error) {
//       console.log(error);
//       if (error.response.data.valid_from) {
...
//   return (
//     <>
//       <h1>This is Edit product offer</h1>
//       <form onSubmit={editProductOffer}>
//         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//           {/* Code */}
